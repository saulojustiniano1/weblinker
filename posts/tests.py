...
class PostAPITestCase(TestCase):
    ...
    def test_create_post(self):
        response = self.client.post(
            '/api/posts/', {'title': 'Novo Post', 'content': 'Conteúdo do novo post'}, HTTP_AUTHORIZATION=self.authorization_header)
    ...
    #         f'/api/posts/{post.id}/', {'title': 'Post Atualizado'})
    #     self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_create_post_unauthenticated(self):
        response = self.client.post('/api/posts/', {'title': 'Novo Post', 'content': 'Conteúdo do novo post'},
                                    HTTP_AUTHORIZATIO='')
